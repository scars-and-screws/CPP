...
int getLength(Box b)
{
    return b.length; // Accessing private data member length
}
...
public:
    // Constructor to initialize car69 attributes
    Car69(string m, int y) : model(m), year(y) {}

    // Declare 'Mechanic' as a friend class
    friend class Mechanic; // Friend class declaration
};

class Mechanic
{
public:
    // Mechanic has access to Car's private members due to friendship
    void displayCarDetails(Car69 &car69)
    {
        cout << "Car Model: " << car69.model << ", Year: " << car69.year << endl;
    }
};

// ! Class Templates (Generic Classes)
// Class template to represent a simple storage of any data type
template <typename T>
class Storage
{
private:
...
public:
    ...
    int getValue()
    {
        cout << "Getting the integer value: ";
        return value;
    }
...
int main(int argc, char const *argv[])
{
    ...
    // * A friend function is a function that is not a member of a class but has access to its private and protected members of the class.
    Box b(10);                                         // Create a Box object
    cout << "Length of Box: " << getLength(b) << endl; // Access private data member length using friend function

    // ? Friend Class
    // * A friend class is a class that is not a member of another class but has access to its private and protected members.
    // * A friend class can access the private and protected members of the class it is a friend of.
    Car69 car1("Ford", 2020); // Create a Car object
    Mechanic mechanic;        // Create a Mechanic object
    mechanic.displayCarDetails(car1);

    // ! Class Templates
    // * A class template is a blueprint for creating classes based on generic types. that can work with any data type.
    //  *   template <typename T>: Declares a class template with a type placeholder T.
    // * The type T will be specified when the class is instantiated.

    Storage<int> intStorage(42);         // Storage object for int
    Storage<double> doubleStorage(3.14); // Storage object for double

    cout << "Integer storage: " << intStorage.getValue() << endl;
    cout << "Double storage: " << doubleStorage.getValue() << endl;

    intStorage.setValue(100);
    cout << "Updated Integer storage: " << intStorage.getValue() << endl;

    // ? Template specialization
    // * Template specialization allows us to define a different implementation for a template when a specific type is passed as a template argument.

    //? int specialization
    Storage<int> intStorage2(42);
    cout << intStorage2.getValue() << endl;

    // ! Template with multiple parameters
    // * A class template can have multiple type parameters separated by commas.
    // * The type parameters can be used to define the data members and member functions of the class.
